function cardCounter(array) {
  // declare count variable at 0
    var count = 0;
    // iterate over the array
    for (var i = 0; i < array.length; i++) {
      // set the card value to array[i]
        var card = array[i];
        var cardValues = (Object.values(card)[0]);
        if (cardValues >= 2 && cardValues <= 6) {
            count++;
        } else if (cardValues === 10 || cardValues === 'face or ace') {
            count--;
        }
